package monitor

import (
	"os"
	"time"

	"github.com/irisnet/ibc-explorer-openapi/internal/app/task"
	"github.com/prometheus/client_golang/prometheus"
	"github.com/sirupsen/logrus"
)

var (
	cronTaskStatusMetric *prometheus.GaugeVec
	TagName              = "taskname"
)

func NewMetricCronWorkStatus() {
	cronTaskStatusMetric = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Namespace: "iobscan_ibc",
		Subsystem: "openapi",
		Name:      "cron_task_status",
		Help:      "ibc_openapi cron task working status (1:Normal  -1:UNormal)",
	}, []string{TagName})
	prometheus.MustRegister(cronTaskStatusMetric)
}

func clientStatus(quit chan bool) {
	for {
		t := time.NewTimer(time.Duration(10) * time.Second)
		select {
		case <-t.C:
			for _, taskName := range []string{
				new(task.IBCChainFeeStatisticTask).Name(),
			} {
				if value, ok := task.TaskMetricMap.Load(taskName); ok {
					cronTaskStatusMetric.WithLabelValues(TagName, taskName).Set(value.(float64))
				} else {
					cronTaskStatusMetric.WithLabelValues(TagName, taskName).Set(0)
				}

			}
		case <-quit:
			logrus.Debug("quit signal recv redisClientStatus")
			return
		}
	}
}

func Start(port string) {
	quit := make(chan bool)
	defer func() {
		close(quit)
		if err := recover(); err != nil {
			logrus.Error("monitor server occur error ", err)
			os.Exit(1)
		}
	}()
	logrus.Info("monitor server start")
	// start monitor
	NewMetricCronWorkStatus()
	clientStatus(quit)
}
